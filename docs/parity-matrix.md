...
>
> **Personal use — all limits removed (verified 2026-08-21):** single-tenant private deployment, no pricing/plans, unlimited domains/subscribers/campaigns/manual IDs 1M/variants 20/tags unlimited/import 100k/fetch 10M. Limiting/gating rows are 🟢 (business tier gating removed); feature rows keep their true status (see the totals footer — not every row is implemented). See `docs/lumapush-deep-research.md`.
...
| B2 | Fetch content from URL (og-scrape) | fetch-content service (SSRF-safe) | 🟢 editor prefill button |
...
| B9 | Clone campaign | clone action | 🟢 duplicate action with A/B carry-over |
| B10 | Clone domain config (multi-site ease) | domain clone | ⚪ |
| B11 | Instant delivery notice (real-time) | SSE live feed ➕ | 🟢 /api/live + dashboard feed |
...
| E7 | A/B title testing | 50/50 split + per-variant stats | 🟢 |
| E8 | Best-send-time heatmap | click-hour heatmap | 🟢 |
...
| F5 | YouTube links (video description collection) | YT links | 🟢 channels module + LP landing pages + polling |
...
| G3 | Worker count (advanced) | worker count | ⚪ (concurrency knob shipped; multi-process pending) |
...
| G7 | Daily cleanup toggle | cleanup job + retention (0 = off) | 🟢 |
| G8 | API access toggle | API toggle | 🟢 keys stop working while off |
...
| H5 | REST API for developers (unlimited calls) | API keys + rate limit | 🟢 v1 keys (hashed, scoped, expiring) + stats/send/trigger |
| H6 | Segment sending via API | API send | 🟢 /api/v1/send (all/manual/segment) |
| H7 | Analytics via REST | API stats | 🟢 /api/v1/stats |
