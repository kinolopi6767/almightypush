...
describe("isPrivateIp", () => {
  it("flags private IPv4 ranges", () => {
    for (const ip of ["10.0.0.1", "172.16.5.5", "192.168.1.1", "127.0.0.1", "169.254.1.2", "0.0.0.1", "100.64.0.1", "192.0.0.10", "198.18.0.1", "198.51.100.7", "203.0.113.7", "224.0.0.1"]) {
      expect(isPrivateIp(ip), ip).toBe(true);
    }
  });

  it("accepts public IPv4 addresses", () => {
    for (const ip of ["8.8.8.8", "1.1.1.1", "9.9.9.9", "172.32.0.1", "100.128.0.1", "198.51.101.7", "203.0.114.7"]) {
      expect(isPrivateIp(ip), ip).toBe(false);
    }
  });

  it("flags private IPv6 ranges", () => {
    for (const ip of ["::1", "::", "fc00::1", "fdf8::1", "fe80::1", "febf:abcd::1", "2001:db8::1", "2001:db8:1234::", "2001:10::1", "2001:20::1", "64:ff9b::1", "100::1", "ff02::1", "ff05::1:3", "FF02::1"]) {
      expect(isPrivateIp(ip), ip).toBe(true);
    }
  });
