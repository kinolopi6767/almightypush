...
const ICON_PATHS: Record<string, string> = {
 ...
 "/dashboard/workspaces": "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z",
};
...
function NavLink({ href, label, active }: { href: string; label: string; active: boolean }) {
 ...
 return (
  <>
   <NavIcon href={href} className="size-4 shrink-0 opacity-80" />
   <span className="rail-label truncate">{label}</span>
   {pending && (
    <span aria-hidden className="rail-label ml-auto size-3 shrink-0 animate-spin rounded-full border-[1.5px] border-current border-t-transparent opacity-70" />
   )}
   {active && <span aria-hidden className="rail-label ml-auto size-1.5 shrink-0 rounded-full bg-[var(--brand)]" />}
  </>
 );
}
  ...
  <nav className="flex flex-col gap-5" aria-label="Primary">
   {SECTIONS.map((section) => (
    <div key={section.heading}>
     <p className="console-navgroup rail-group">{section.heading}</p>
      ...
      {section.items.map((item) => {
       ...
       return (
        <Link
         ...
         title={item.label}
         className="console-navitem group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)]"
        >
