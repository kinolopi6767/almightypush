...
export default async function TeamPage() {
 ...
 if (!session?.user) redirect("/login");
  ...
  const invites = wsId
    ? db
        ...
        .from(teamInvites)
        .where(eq(teamInvites.workspace_id, wsId))
        .all()
    : [];
 ...
 return (
  <div className="space-y-6">
   <PageHeader eyebrow="System · Team"
    title="Team"
   ...
   <div className="panel p-4">
    ...
    {invites.length === 0 ? (
     ...
     <ul className="mt-3 space-y-1.5">
      {invites.map((i) => (
       <li key={i.id} className="flex flex-wrap items-center gap-2 rounded-md border px-3 py-2 text-sm">
