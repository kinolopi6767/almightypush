...
export default async function LogsPage({
  ...
  if (q) {
    // Sanitize LIKE wildcards: Escape % and _ to prevent unintended matches
    const sanitized = q.replace(/[%_]/g, "\\$&");
    conditions.push(like(auditLog.action, `%${sanitized}%`));
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <a
            ...
            className="inline-flex h-9 items-center rounded-xl border bg-card px-4 text-sm font-medium shadow-xs hover:bg-accent"
          >
            Export audit CSV
          </a>
        }
      ...
      {/* Premium filter bar */}
      <form method="get" className="premium-card flex flex-wrap items-end gap-3 rounded-xl p-4">
        <div className="flex flex-col gap-1.5 min-w-0 flex-1 max-w-xs">
          ...
          <input
            ...
            className="h-9 rounded-lg border border-input bg-card px-3 text-sm shadow-xs focus:border-primary/40 focus:ring-2 focus:ring-ring/30 outline-none"
          />
        </div>
        <div className="flex flex-col gap-1.5">
          ...
          <select
            ...
            className="h-9 rounded-lg border border-input bg-card px-3 text-sm shadow-xs"
          >
        ...
        <button type="submit" className="h-9 rounded-xl bg-primary px-5 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary-hover">
          Filter
        </button>
        {(q || filterAction) && (
          <a href="/dashboard/logs" className="h-9 inline-flex items-center rounded-xl border bg-card px-4 text-sm shadow-xs hover:bg-accent">
            Clear
          </a>
        )}
      </form>
      ...
      <div className="premium-card overflow-hidden rounded-xl">
        <div className="border-b bg-muted/20 px-5 py-3 flex items-center justify-between">
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-success/10 px-2.5 py-1 text-xs font-medium text-success ring-1 ring-success/15">
            <span className="size-1.5 rounded-full bg-success pulse-dot" aria-hidden /> Live audit
          </span>
        </div>
        ...
        {rows.length === 0 ? (
          ...
          <div className="max-h-[600px] overflow-y-auto premium-scroll divide-y divide-border/60">
            {rows.map((r) => (
              <div key={r.id} className="flex items-start gap-3 px-5 py-3 hover:bg-muted/20 transition-colors">
                <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset ${levelBadge(r.action)}`}>
                  {r.action}
                </span>
                {r.entity_type && (
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="premium-card rounded-xl p-5">
          <p className="kicker text-muted-foreground">Log levels</p>
        ...
        <div className="premium-card rounded-xl p-5">
          <p className="kicker text-muted-foreground">Retention</p>
        ...
        <div className="premium-card rounded-xl p-5">
          <p className="kicker text-muted-foreground">Export</p>
