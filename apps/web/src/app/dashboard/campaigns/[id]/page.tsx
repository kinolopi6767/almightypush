...
import { RetryFailedForm } from "./retry-form";
...
export default async function CampaignDetailPage({ params }: Props) {
 ...
 const byStatusRows = db
  ...
  .all();
 const byStatus = new Map<string, number>(byStatusRows.map((r) => [r.status, r.value]));
 ...
 return (
  <>
   ...
   {campaign.title_b && (
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
     {(["a", "b"] as const).map((v) => {
      ...
      return (
       <div key={v} className={`panel p-4 ${isWinner ? "ring-2 ring-emerald-500/70" : ""}`}>
   ...
   <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
    <div className="space-y-6">
     ...
     <div className="panel p-4">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
       Showing {deliveryRows.length} of the latest — whole-campaign totals:{" "}
    ...
    <RetryFailedForm campaignId={campaign.id} status={campaign.status} failedCount={failedCount} />
