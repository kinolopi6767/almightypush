...
export default async function SettingsPage() {
 ...
 return (
  <div className="space-y-8">
   <PageHeader eyebrow="System · Settings" title="Settings" description="Panel-wide configuration and database backups." />
       ...
       <ul className="divide-y divide-border/60">
        {auditRows.map((row) => (
         <li key={row.id} className="flex items-center gap-3 px-4 py-3 text-sm hover:bg-muted/30 transition-colors">
          <code className="shrink-0 rounded-md bg-muted px-2 py-1 font-mono text-xs font-medium border">{row.action}</code>
          {row.entity_type && (
