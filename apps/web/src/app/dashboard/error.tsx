...
export default function DashboardError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="premium-card mx-auto max-w-lg rounded-2xl p-8 text-center" role="alert" aria-live="assertive">
      <span
        aria-hidden
        className="mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl bg-destructive/10 text-destructive ring-1 ring-inset ring-destructive/20 shadow-xs"
      >
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-center">
        <button
          onClick={() => reset()}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-medium text-primary-foreground shadow-[0_4px_16px_-4px_color-mix(in_oklab,var(--primary)_55%,transparent)] transition-all hover:bg-primary-hover hover:shadow-[0_6px_20px_-4px_color-mix(in_oklab,var(--primary)_55%,transparent)] hover:-translate-y-px active:translate-y-0 active:scale-[0.98]"
        >
        ...
        <a
          href="/dashboard"
          className="inline-flex h-10 items-center justify-center rounded-xl border bg-card px-5 text-sm font-medium shadow-xs transition-colors hover:bg-accent hover:border-border-strong"
        >
          Go to dashboard
        </a>
      </div>
