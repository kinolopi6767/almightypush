...
export default async function SegmentsPage() {
 ...
 return (
  <>
   <PageHeader eyebrow="Grow · Segments" title="Segments" description="Reusable audience rules built on subscriber attributes. Pick a segment as a campaign audience and it stays up to date." />
   ...
   <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_420px]">
    <div className="space-y-3">
     ...
     {rows.map((row) => {
      ...
      return (
       <div key={row.id} data-entity="segment" data-testid={`row-segment-${row.id}`} className=" rounded-md border bg-card p-5">
        <div className="flex items-start justify-between gap-3">
         <div className="min-w-0 flex-1">
          <p className="truncate font-medium">{row.name}</p>
          <p className="mt-0.5 truncate text-sm text-muted-foreground">{domainsLabel}</p>
         </div>
         <span className="shrink-0 whitespace-nowrap badge badge-ok tabular-nums">
          ~{row.estimate_count?.toLocaleString() ?? "…"} subs
         </span>
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
         <Link href={`/dashboard/segments/${row.id}`} className="btn btn-secondary btn-sm" aria-label={`Edit segment ${row.name}`}>
          Edit
         </Link>
         <form action={deleteSegmentAction.bind(null, row.id)}>
          <SubmitButton confirm={`Delete segment "${row.name}"?`} pendingLabel="Deleting…" title={`Delete segment ${row.name}`} className="btn btn-danger-ghost btn-sm">
           Delete
          </SubmitButton>
         </form>
