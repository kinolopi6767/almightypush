  ...
  return (
    <div className="space-y-6">
      ...
      <div className="surface rounded-xl p-5">
        ...
        {recent.length === 0 ? (
          <p className="mt-4 rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No email campaigns yet — create one from the panel or via API.</p>
        ) : (
          <pre className="mt-3 max-h-96 overflow-auto rounded-lg bg-muted p-3 text-xs leading-relaxed">{JSON.stringify(recent, null, 2)}</pre>
        )}
      </div>
    </div>
  );
