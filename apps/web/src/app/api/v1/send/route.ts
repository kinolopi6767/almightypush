...
export async function POST(req: Request) {
  ...
  try {
    ...
    return corsJson({ ok: false, error: "Invalid JSON body" }, { status: 400 });
  }
  if (!body || typeof body !== "object") {
    return corsJson({ ok: false, error: "Invalid body" }, { status: 400 });
  }

  const title = typeof body.title === "string" ? body.title.trim() : "";
  if (!title || title.length > MAX_TITLE) {
    return corsJson({ ok: false, error: `title is required (max ${MAX_TITLE} chars)` }, { status: 400 });
  }
  const titleB = typeof body.title_b === "string" && body.title_b.trim() ? body.title_b.trim() : null;
  if (titleB && titleB.length > MAX_TITLE) {
    return corsJson({ ok: false, error: `title_b too long (max ${MAX_TITLE} chars)` }, { status: 400 });
  }
  const message = typeof body.message === "string" && body.message.trim() ? body.message.trim() : null;
  if (message && message.length > MAX_MESSAGE) {
    return corsJson({ ok: false, error: `message too long (max ${MAX_MESSAGE} chars)` }, { status: 400 });
  }
  const url = typeof body.url === "string" && body.url.trim() ? body.url.trim() : null;
  if (url && !isValidHttpUrl(url)) return corsJson({ ok: false, error: "url must be http(s)" }, { status: 400 });
  const iconUrl = typeof body.icon_url === "string" && body.icon_url.trim() ? body.icon_url.trim() : null;
  if (iconUrl && !isValidHttpUrl(iconUrl)) return corsJson({ ok: false, error: "icon_url must be http(s)" }, { status: 400 });
  const imageUrl = typeof body.image_url === "string" && body.image_url.trim() ? body.image_url.trim() : null;
  if (imageUrl && !isValidHttpUrl(imageUrl)) return corsJson({ ok: false, error: "image_url must be http(s)" }, { status: 400 });

  const buttonsRaw = Array.isArray(body.buttons) ? body.buttons : [];
  if (buttonsRaw.length > MAX_BUTTONS) {
    return corsJson({ ok: false, error: `at most ${MAX_BUTTONS} buttons` }, { status: 400 });
  }
  ...
  for (const b of buttonsRaw) {
    ...
    if (!BUTTON_LABEL_RE.test(label) || !isValidHttpUrl(bUrl)) {
      return corsJson({ ok: false, error: "button label (max 24 chars) and http(s) url required" }, { status: 400 });
    }
    if (seenLabels.has(label)) return corsJson({ ok: false, error: `duplicate button label "${label}"` }, { status: 400 });
  ...
  const domainRows = typeof domainParam === "number"
    ...
    : domainNumeric !== null
      ...
      : domainTrimmed
        ...
        : [];
  if (domainRows.length === 0) return corsJson({ ok: false, error: "domain not found in this workspace" }, { status: 404 });
  const domain = domainRows[0]!;
  if (!domainAllowed(auth.context, domain.id)) {
    return corsJson({ ok: false, error: "domain not covered by this key" }, { status: 403 });
  }
  if (domain.status !== "active") {
    return corsJson({ ok: false, error: "domain is not active" }, { status: 409 });
  }
  ...
  if (audience.kind === "manual") {
    ...
    if (ids.length === 0) return corsJson({ ok: false, error: "manual audience needs at least one id" }, { status: 400 });
    if (ids.length > MAX_MANUAL_IDS) return corsJson({ ok: false, error: `manual audience capped at ${MAX_MANUAL_IDS} ids` }, { status: 400 });
    ...
    if (!Number.isInteger(segmentId) || !(segmentId! > 0)) {
      return corsJson({ ok: false, error: "segment_id (positive integer) required" }, { status: 400 });
    }
    const [segment] = db
      ...
      .limit(1)
      .all();
    if (!segment) return corsJson({ ok: false, error: "segment not found" }, { status: 404 });
    ...
    return corsJson({ ok: false, error: "audience.kind must be all | manual | segment" }, { status: 400 });
  }
  ...
  if (body.schedule) {
    if (typeof body.schedule !== "string" || !ISO_RE.test(body.schedule)) {
      return corsJson({ ok: false, error: "schedule must be an ISO timestamp" }, { status: 400 });
    }
    const t = Date.parse(body.schedule);
    if (Number.isNaN(t)) return corsJson({ ok: false, error: "schedule is not a valid date" }, { status: 400 });
  ...
  if (body.ttl !== undefined) {
    ...
    if (!Number.isFinite(n) || n < 0 || n > 2419200) return corsJson({ ok: false, error: "ttl must be 0-2419200" }, { status: 400 });
    ttl = Math.floor(n);
  }
  let urgency: string | null = null;
  if (body.urgency !== undefined) {
    const u = String(body.urgency).trim().toLowerCase();
    if (!["very-low", "low", "normal", "high"].includes(u)) return corsJson({ ok: false, error: "urgency must be very-low|low|normal|high" }, { status: 400 });
    urgency = u;
  }
  ...
  if (body.variants !== undefined) {
    if (!Array.isArray(body.variants) || body.variants.length < 2 || body.variants.length > MAX_VARIANTS) {
      return corsJson({ ok: false, error: `variants must be array of 2-${MAX_VARIANTS}` }, { status: 400 });
    }
    ...
    for (let i = 0; i < body.variants.length; i++) {
      ...
      if (!vt || vt.length > MAX_TITLE) return corsJson({ ok: false, error: `variant ${i} title required max ${MAX_TITLE}` }, { status: 400 });
      ...
      if (vi && !isValidHttpUrl(vi)) return corsJson({ ok: false, error: `variant ${i} image_url must be http(s)` }, { status: 400 });
      const w = v?.weight !== undefined ? Number(v.weight) : 10;
      if (!Number.isFinite(w) || w < 1 || w > 100) return corsJson({ ok: false, error: `variant ${i} weight 1-100` }, { status: 400 });
