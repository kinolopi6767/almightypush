...
export async function POST(req: Request) {
  ...
  for (const b of buttonsRaw) {
    ...
    if (!BUTTON_LABEL_RE.test(label) || !URL_RE.test(bUrl)) {
  ...
  // Domain resolution: id or name, must belong to the workspace and be
  // covered by the key's domain scope.
  ...
  const domainRows = typeof domainParam === "number"
    ...
    : typeof domainParam === "string" && domainParam.trim()
      ? db.select().from(domains).where(and(eq(domains.name, domainParam.trim()), eq(domains.workspace_id, auth.context.workspaceId))).limit(1).all()
      : [];
