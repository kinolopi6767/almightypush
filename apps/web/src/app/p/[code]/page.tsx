...
export default async function LandingPage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ dev?: string }> }) {
 ...
 if (link.deleted_at) {
  // Tombstoned links redirect to the operator's fallback URL — still
  // restricted to http(s) so a poisoned row can't become a javascript: sink.
  try {
   const protocol = new URL(link.target_url).protocol;
   if (protocol !== "http:" && protocol !== "https:") notFound();
  } catch {
   notFound();
  }
 ...
 return (
  <>
   ...
   <style>{`body{margin:0;background:#0A1511}`}</style>
   <LandingClient
    code={code}
    baseUrl={baseUrl}
