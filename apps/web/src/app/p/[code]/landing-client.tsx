...
export function LandingClient({ code, baseUrl, targetUrl, prompt, forceSubscribe, domainId, publicKey, devMode }: LandingClientProps) {
 ...
 const go = useCallback(
  (subscribed: boolean) => {
   // Protocol allowlist: creation validates http(s), but a legacy or
   // hand-edited row with javascript:/data: must never execute here.
   let url: URL;
   try {
   ...
   setState((result as "denied" | "subscribed" | "error") ?? "error");
    if (result === "subscribed") {
      ...
      if (forceSubscribe) {
        ...
        setState("denied");
        setBusy(false);
        const t = setTimeout(() => void finish(false), 4000);
 ...
 useEffect(() => {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 440, width: "100%", textAlign: "center", border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, padding: "36px 28px", background: "var(--rail-2)" }}>
        <div
          style={{
            width: 52,
            height: 52,
        ...
        <p style={{ margin: 0, fontFamily: "monospace", fontSize: 11, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--rail-muted)" }}>
          Push notification signup
        </p>
        <h1 style={{ fontSize: 22, margin: "8px 0 0", letterSpacing: "-.02em" }}>{prompt}</h1>
        <p style={{ margin: "10px 0 0", color: "var(--rail-muted)", fontSize: 13.5, lineHeight: 1.6 }}>
          We&apos;ll only send you updates you care about. Unsubscribe anytime.
        </p>

        {state === "denied" && (
          <p role="alert" style={{ color: "#fca5a5", marginTop: 16, fontSize: 13.5 }}>
